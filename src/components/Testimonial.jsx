...
export default function Testimonial() {
  return (
    <div className="mt-40 tracking-wide" id="testimonials">
      <h2 className="text-3xl sm:text-5xl lg:text-6xl text-center my-10 lg:my-20">What people are saying</h2>
      <div className="flex flex-wrap justify-center">
        {testimonials.map((testimonial, index) => {
          const direction = index < 3 ? "down" : "top";
          const delay = index < 3 ? 0.2 : 0.5;

          return (
            <motion.div key={index} className="w-full sm:w-1/2 lg:w-1/3 px-4 py-2" variants={fadeIn(direction, delay)} initial="hidden" whileInView="show" viewport={{ once: false, amount: 0.3 }}>
